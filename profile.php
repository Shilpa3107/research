...
if (isset($_POST['upload_avatar'])) {
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["avatar_file"]["tmp_name"], $target_file)) {
            // File uploaded successfully, save filename to database
            ...
            // Use prepared statement to update avatar filename
            $sql = "UPDATE registerinfo SET avatar_filename = ? WHERE emp_id = ?";
            $stmt = $conn->prepare($sql);

            // Bind parameters to the prepared statement
            $stmt->bind_param("si", $avatar_filename, $employee_id);

            if ($stmt->execute()) {
               // echo '<script>alert("Avatar uploaded successfully")</script>';
            } else {
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>User Profile Page - Amity University</title>
											...
											<div>
												...
												if (isset($_SESSION['employee_id'])) {
													...
													if ($result && $result->num_rows > 0) {
														$row = $result->fetch_assoc();
														$avatar_filename = $row['avatar_filename'];
														echo '<img id="avatar" class="img-responsive" alt="User Avatar" src="uploads/' . $avatar_filename . '">';
													} else {
														// If no avatar found, display default avatar
														echo '<img id="avatar" class="img-responsive" alt="Default Avatar" src="assets\images\avatars\default-avatar.jpg">';
													}
												...
												<p>Choose file to Upload Profile Picture</p>
